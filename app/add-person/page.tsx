...
export default function AddPersonPage() {
  ...
  const isTeamLead  = myDbRole === "Team Lead";
  const canManageTeams = isAdmin || isTeamLead;
  ...
  const handleEdit = (p: Person) => {
    setEditingId(p.id);
    setName(p.name); setEmail(p.email); setPersonRole(p.role);
  ...
  const toggleMember = async (teamId: number, personId: number, currentlyIn: boolean) => {
    await fetch("/api/teams", {
      ...
      body: JSON.stringify(
        currentlyIn
          ? { teamId, removePersonIds: [personId] }
          : { teamId, addPersonIds: [personId] }
      ),
    });
    // Update both teams and people locally
    setTeams((prev) =>
      prev.map((t) => {
  ...
  return (
    <>
      ...
      <div style={pageStyle}>
        ...
        {/* ── Viewer banner ── */}
        {myRole === "viewer" && myDbRole === "Employee" && (
        ...
        {canManageTeams && (
          <div style={cardStyle}>
            ...
            {teams.length === 0 ? (
              <p style={emptyStyle}>No teams yet. Create one above.</p>
            ) : (
                  ...
                  <div key={team.id} style={teamCardStyle}>
                    ...
                    {team.members.length > 0 && managingTeamId !== team.id && (
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.375rem", marginTop: "0.625rem" }}>
                        {team.members.map((m) => (
                          <span key={m.id} style={memberChipStyle}>{m.name}</span>
                        ))}
                      </div>
                    )}
                            ...
                            return (
                              <label key={p.id} style={memberToggleStyle(inTeam)}>
                                <input
                                  type="checkbox"
                                  checked={inTeam}
                                  onChange={() => toggleMember(team.id, p.id, inTeam)}
                  ...
                  <div key={person.id} style={personRowStyle}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
                        ...
                        {person.teams.map((t) => (
                          <span key={t.id} style={teamTagStyle}>{t.name}</span>
                        ))}
                      </div>
...
const pageStyle: React.CSSProperties = {
  maxWidth: "820px", margin: "0 auto", padding: "6rem 1.5rem 3rem",
...
const inputStyle: React.CSSProperties = {
  ...
  fontSize: "0.9rem", color: "#0f172a", outline: "none", background: "#f8fafc",
...
const dayChipStyle: React.CSSProperties = {
  padding: "0.4rem 0.875rem", borderRadius: "999px", border: "1.5px solid #e2e8f0",
  cursor: "pointer", fontSize: "0.82rem", fontWeight: 500, color: "#64748b",
...
const viewerBannerStyle: React.CSSProperties = {
  ...
  borderRadius: "12px", padding: "0.875rem 1.25rem", fontSize: "0.875rem", fontWeight: 500,
};
const filterRowStyle: React.CSSProperties = {
  display: "flex", gap: "0.75rem", flexWrap: "wrap",
};
const badgeStyle: React.CSSProperties = {
  background: "#f1f5f9", color: "#64748b", borderRadius: "999px",
  padding: "0.25rem 0.75rem", fontSize: "0.8rem", fontWeight: 500,
};
const emptyStyle: React.CSSProperties = { color: "#94a3b8", fontSize: "0.875rem", margin: 0 };
const teamCardStyle: React.CSSProperties = {
  border: "1px solid #e2e8f0", borderRadius: "12px", padding: "0.875rem 1.125rem", background: "#fafafa",
};
...
const memberChipStyle: React.CSSProperties = {
  ...
  padding: "0.2rem 0.625rem", fontSize: "0.78rem", fontWeight: 500,
};
const managePanelStyle: React.CSSProperties = {
  marginTop: "0.875rem", paddingTop: "0.875rem",
  borderTop: "1px solid #e2e8f0",
};
const memberToggleStyle = (active: boolean): React.CSSProperties => ({
...
const avatarStyle: React.CSSProperties = {
  ...
  background: "linear-gradient(135deg,#1a202c,#4a5568)", color: "white",
  display: "flex", alignItems: "center", justifyContent: "center",
  fontWeight: 700, fontSize: "1rem", flexShrink: 0,
};
const roleTagStyle: React.CSSProperties = {
  background: "#f1f5f9", color: "#475569", borderRadius: "999px",
  padding: "0.1rem 0.6rem", fontSize: "0.75rem", fontWeight: 500,
};
...
const editBtnStyle: React.CSSProperties = {
  ...
  border: "1.5px solid #e2e8f0", borderRadius: "8px", fontSize: "0.8rem", fontWeight: 500, cursor: "pointer",
};
...
const spinnerStyle: React.CSSProperties = {
  ...
  borderTopColor: "#0f172a", borderRadius: "50%", animation: "spin 0.7s linear infinite",
};
